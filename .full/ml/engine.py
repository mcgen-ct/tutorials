...
class Scalar:
    def __init__(self, data, _children = (), _op = "", label = None):
        ...
        else: self.label = f"Scalar({data:.4g})"
    ...
    def __pow__(self, power):
        if not isinstance(power, Scalar): power = Scalar(power)
    ...
    def ifel(self, condition, true_case, false_case):
        if not isinstance(true_case, Scalar): true_case = Scalar(true_case)
        if not isinstance(false_case, Scalar): false_case = Scalar(false_case)

        output = Scalar(true_case.data if condition else false_case.data, _children=(self,), _op='ifel')
    ...
    def visualize_graph(self, output_label = None):
        ...
        for n in nodes:
            if output_label is not None: graph.node(name=str(id(n)), label = f"{n.label} | data: {n.data:.4f} | D[{output_label}, {n.label}]: {n.grad:.4f}", shape='record', style='filled', fillcolor='white')
            else: graph.node(name=str(id(n)), label = f"{n.label} | data: {n.data:.4f} | grad: {n.grad:.4f}", shape='record', style='filled', fillcolor='white')
            if n._op:
