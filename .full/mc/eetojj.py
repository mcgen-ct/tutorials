...
class eeTojj:
    ...
    def ME2(self,fl,s,t):
        ...
        qe = -1.          # Electric charge of electron
        ae = -0.5         # Axial-vector coupling of electron
        ve = ae - 2. * qe * self.sin2tw  # Vector coupling of electron

        # Final state fermion charge and couplings
        qf = 2. / 3. if fl in [2, 4] else -1. / 3.      # Electric charge of final state fermion
        af = 0.5 if fl in [2, 4] else -0.5              # Axial-vector coupling of final state fermion
        vf = af - 2. * qf * self.sin2tw                 # Vector coupling of final state fermion
        ...
        chi1 = kappa * s * (s - self.MZ2) / (np.power(s - self.MZ2, 2.) + self.GZ2 * self.MZ2)
        chi2 = np.power(kappa * s, 2.) / (np.power(s - self.MZ2, 2.) + self.GZ2 * self.MZ2)

        # Matrix element squared terms
        term1 = (1 + np.power(1. + 2. * t / s, 2.)) * (np.power(qf * qe, 2.) + 2. * (qf * qe * vf * ve) * chi1 + (ae * ae + ve * ve) * (af * af + vf * vf) * chi2)
        term2 = (1. + 2. * t / s) * (4. * qe * qf * ae * af * chi1 + 8.* ae * ve * af * vf * chi2)
        return np.power(4. * np.pi * self.alpha, 2.) * 3. * (term1 + term2)
    ...
    def generate_event(self):
        ...
        ct = 2. * np.random.random() - 1.           # cos(theta) uniformly in [-1, 1]
        st = np.sqrt(1. - ct * ct)                  # sin(theta) from cos(theta)
        phi = 2. * np.pi * np.random.random()       # phi uniformly in [0, 2pi)
        ...
        pab_plus_M2 = (pa[0] + pb[0])**2 - (pa[1] + pb[1])**2 - (pa[2] + pb[2])**2 - (pa[3] + pb[3])**2
        pa1_minus_M2 = (pa[0] - p1[0])**2 - (pa[1] - p1[1])**2 - (pa[2] - p1[2])**2 - (pa[3] - p1[3])**2
...
if __name__ == '__main__':
    ...
    for i in range(nevents):
        ...
        for particle in ev[0]:
            print(f"  Particle ID: {particle.data.pid}, Momentum: {particle.p}, Color: {particle.c if hasattr(particle, 'c') else 'N/A'}")
