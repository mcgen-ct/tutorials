...
class Visualization:
    ...
    def compute_mid_end_pos(self, p, start_position,
                            category=None,
                            scale_factor=None, rescale = False,
    ...
    def get_initial_particles(self):
        ...
        for i in range(self.pythia.event.size()):
            ...
            if category in ["hard_process", "beam_remnants", "MPI"]:
                ...
                if ((category == "beam_remnants" and 
                     self.settings.get("beam_remnant", False)) or
                    (category == "MPI" and 
                     self.settings.get("mpi", False)) or
                    (category == "hard_process")):
    ...
    def get_descendants(self, prt_idx, depth=0, effective_mother=None):
        ...
        if abs(status) > 90:
            ...
            d2 = prt.daughter2()
            if d1 > 0:
                descendants += self._process_daughters(d1, d2, depth + 1,
                                                       prt_idx)
        ...
        if self.settings.get("remove_copy", False):
            ...
            if d1 > 0 and d2 == d1 and abs(status) not in [23, 63]:
                ...
                if pid == daughter.id():
                    print(f"{'  '*depth}Carbon Copy Detected: Skipping "
                          f"mother Index {prt_idx}")
                    # Continue processing with daughter; effective mother 
                    # remains the same.
                    descendants += self._process_daughters(d1, d1, depth,
                                                            effective_mother)
                    return descendants, self.particle_mother_mapping
        ...
        if (mother_energy is not None and
                energy > mother_energy + tolerance):
            print(f"{'  ' * depth}Warning: Particle {prt_idx} has more "
        ...
        print(f"{'  ' * depth}{depth} Particle Index: {prt_idx}, "
              f"PDG ID: {pid}, Name: {particle_name}, Status: {status}, "
              f"Category: {category}")
        print(f"{'  ' * depth}  Energy: {energy:.2e} GeV, px: {p.px():.2e}, "
        ...
        if abs(pid) == 21 and (d1 > 0 or d2 > 0):
            daughters = []
            if d1 > 0: daughters.append(d1)
            if d2 > 0 and d2 != d1: daughters.append(d2)
            ...
            if not can_proceed:
                print(f"{'  '*depth}Skipping gluon {prt_idx}, daughter(s) already claimed by higher-energy gluon")
                return descendants, self.particle_mother_mapping
    ...
    def assig_hadron_to_parton(self, boost=None):
        ...
        for hadron_index, mother_indices in (
            self.particle_mother_mapping.items()
            ):
            ...
            if 80 < abs(status) < 90 and len(mother_indices) > 1:
                print(f"Processing hadron index {hadron_index} "
                ...
                for mother_index in mother_indices:
                    ...
                    print(f"  Mother index {mother_index}, delta R: {dR:.4f}, "
                    ...
                    if cos_theta_value > 0:
                        ...
                        if (dR < smallest_delta_r 
                            or (dR == smallest_delta_r 
                            and cos_theta_value > best_cos_theta)
                            ):
                            smallest_delta_r = dR
                ...
                if best_mother_index is None and (
                                fallback_mother_index is not None):
                    best_mother_index = fallback_mother_index
                    smallest_delta_r = fallback_smallest_delta_r
                    print(f"  No positive cos(theta). Fallback to mother "
                ...
                if best_mother_index is not None:
                    print(f"  Keeping mother {best_mother_index} "
                          f"with delta R: {smallest_delta_r:.4f}, " 
                          f"cos(theta): {best_cos_theta:.4f}")
                    self.particle_mother_mapping[hadron_index] = [
                                                    best_mother_index
                    ]
                print("-" * 40)
    ...
    def get_color_neutral_object(self):  
        ...
        for prt_idx in parton_list:
            ...
            print(f"Particle {prt_idx}: Status = {status}, "
        ...
        while partons_to_process:
            ...
            while (color_index in anti_color_map and 
                   anti_color_map[color_index]):
            ...
            while (anti_color_index in color_map and 
                   color_map[anti_color_index]):
    ...
    def compute_positions(self, initial_particles, rescale=False, x_shift=None,
                          y_shift=None, z_shift=None):
        """
        ...
        for prt_idx in initial_particles:
            ...
            #    sys.exit()
                ...
                furthest_dist = self.category_furthest_dist[key].get(
                    category, 
                    self.category_furthest_dist[key][scaling_type]
                )
            ...
            end_position, middle_position = self.compute_mid_end_pos(
                                                p, start_position, category=category, scale_factor=None, 
                                                rescale =rescale,
        ...
        for prt_idx, mother_indices in sorted_particles.items():
            ...
            if rescale:
                ...
                scale_factor = (
                    self.settings.get("scale_factor", 1) 
                    / furthest_dist
                )
        ...
        if rescale:
            ...
            for particle in self.particle_positions:
                print(f"Particle index: {particle}, Positions: "
    ...
    def find_furthest_dist_from_positions(self):
        ...
        """
        categories_non_beam = ["hard_process", "parton_shower",
                                 "hadronization", "overall"]
        categories_beam = ["beam_remnants", "parton_shower",
                           "hadronization", "overall"]
        ...
        def update_distances(prt_idx, distance, category_key, 
                             is_beam_remnant):
            """
            ...
            for prt_idx, depth in descendants_with_depth:
                ...
                if prt_idx in self.particle_positions:
                    ...
                    if category_key:
                        ...
                        update_distances(prt_idx, end_distance,
                                         category_key, is_beam_remnant)
                        update_distances(prt_idx, start_distance,
                                         category_key, is_beam_remnant)

                    # Always update for the overall distance category
                    update_distances(prt_idx, end_distance, "overall",
                                     is_beam_remnant)
                    update_distances(prt_idx, start_distance, "overall",
                                     is_beam_remnant)

        # Calculate the difference between furthest and 
        # shortest for each category
        for remnant_key in ["non_beam_remnant", "beam_remnant"]:
            categories = (categories_non_beam 
            ...
            for category in categories:
                ...
                furthest_minus_shortest = category_data["furthest"] - \
                                          category_data["shortest"]

                # Store the difference for this category
                self.category_furthest_dist[remnant_key][category] = \
    ...
    def get_color_connection_info(self, bow_height=8):
        ...
        for k, color_neutral_object in enumerate(self.color_neutral_objects):
            ...
            if self.highlight_category is not None and "color_connection" != self.highlight_category:
            ...
            for i in range(len(color_neutral_object) - 1):
                ...
                from_info = (f"idx: {parton_a}, id: "
                             f"{self.pythia.particleData.name(prt_a.id())} "
                             f"({prt_a.id()}), "
                             f"color: {prt_a.col()}, "
                             f"acol: {prt_a.acol()}")
                to_info = (f"idx: {parton_b}, id: "
        ...
        for prt_idx, positions in self.rescaled_positions.items():
            ...
            if abs(pid) in [11, 13, 15, 12, 14, 16]:
                ...
                if np.linalg.norm(start_to_end_vector) > 0:
                    scale_factor = (furthest_distance - start_distance) / np.linalg.norm(start_to_end_vector)
            ...
            if self.highlight_category is not None and category != self.highlight_category:
                color = self.lighten_color("808080", factor=0.3)

            if abs(pid) in [11, 13, 15, 12, 14, 16]:
                if self.highlight_category is None or category != self.highlight_category:
                    color = self.lighten_color("808080", factor=0.3) 
            ...
            self.json_data["Test, tracks playground"]["Tracks"][category].append(track_info)
            particle_info_list.append(track_info)

        self.json_data["Test, tracks playground"]["Vertices"]["TestVertices"].extend(vertices_list)
    ...
    def merge_color_connections_into_tracks(self):
        ...
        if "TestTracks" not in self.json_data["Test, tracks playground"
            ]["Tracks"]:
            self.json_data["Test, tracks playground"
                ]["Tracks"]["TestTracks"] = []

        if "Collor Connection" not in self.json_data["Test, tracks playground"]["Tracks"]:
            self.json_data["Test, tracks playground"]["Tracks"]["Collor Connection"] = []
    ...
    def get_json(self):
        ...
        (self.list_descendants_with_depth, 
         self.particle_mother_mapping) = self.build_particle_tree(
                                                    initial_particles
                                                    )
        ...
        if self.settings["color_connection"]:
            (self.color_neutral_objects, 
             leftover_partons) = self.get_color_neutral_object()
        ...
        self.category_furthest_dist = \
            self.find_furthest_dist_from_positions()
        
        # Rescale positions with provided shifts.
        self.rescaled_positions = self.compute_positions(initial_particles, 
                                                         rescale=True, 
                                                         x_shift=x_shift, 
                                                         y_shift=y_shift, 
                                                         z_shift=z_shift)
...
def parse_arguments():
    ...
    parser.add_argument(
        "--file_name", type=str, default=default_file_name,
    ...
    if not (args.command_file or args.read_strings):
        parser.error("At least one of --command_file or"
                     " --read_strings must be provided.")
